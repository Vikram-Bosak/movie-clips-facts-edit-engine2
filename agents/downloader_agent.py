...
async def resolve_entries(url: str):
    ...
    cmd = ["yt-dlp", "--extractor-args", "youtube:player_client=ios,web", "--js-runtimes", "node", "--remote-components", "ejs:github", "--flat-playlist", "--playlist-end", "30", "--skip-download", "-J"] + get_cookie_flags() + [url]
    try:
...
async def fetch_full_metadata(video_url: str):
    ...
    cmd = ["yt-dlp", "--extractor-args", "youtube:player_client=ios,web", "--js-runtimes", "node", "--remote-components", "ejs:github", "--skip-download", "-J"] + get_cookie_flags() + [video_url]
    try:
        proc = await asyncio.create_subprocess_exec(
            *cmd,
            stdout=asyncio.subprocess.PIPE,
